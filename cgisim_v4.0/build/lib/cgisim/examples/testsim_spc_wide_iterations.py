...
def testsim_spc_wide_iterations():
    cgi_mode = 'excam'
    ...
    dm1 = proper.prop_fits_read( roman_preflight_proper.lib_dir+'/examples/spc-wide_ni_2e-8_dm1_v.fits' )
    dm2 = proper.prop_fits_read( roman_preflight_proper.lib_dir+'/examples/spc-wide_ni_2e-8_dm2_v.fits' )
    params = {'use_errors':1, 'use_dm1':1, 'dm1_v':dm1, 'use_dm2':1, 'dm2_v':dm2}
    ...
    dm1 = proper.prop_fits_read( roman_preflight_proper.lib_dir+'/examples/spc-wide_ni_5e-9_dm1_v.fits' )
    dm2 = proper.prop_fits_read( roman_preflight_proper.lib_dir+'/examples/spc-wide_ni_5e-9_dm2_v.fits' )
    params = {'use_errors':1, 'use_dm1':1, 'dm1_v':dm1, 'use_dm2':1, 'dm2_v':dm2}
    ...
    im = ax[0,0].imshow( trim(ni_flattened,180), norm=LogNorm(vmin=1e-10,vmax=1e-5), cmap=plt.get_cmap('jet'))
    ax[0,0].set_title('Flattened')
    fig.colorbar( im, ax=ax[0,0], shrink=0.5 ) 

    im = ax[0,1].imshow( trim(ni_worst,180), norm=LogNorm(vmin=1e-10,vmax=1e-5), cmap=plt.get_cmap('jet'))
    ax[0,1].set_title('Early iteration')
    fig.colorbar( im, ax=ax[0,1], shrink=0.5 ) 

    im = ax[1,0].imshow( trim(ni_mild,180), norm=LogNorm(vmin=1e-10,vmax=1e-5), cmap=plt.get_cmap('jet'))
    ax[1,0].set_title('Intermediate iteration')
    fig.colorbar( im, ax=ax[1,0], shrink=0.5 ) 

    im = ax[1,1].imshow( trim(ni_best,180), norm=LogNorm(vmin=1e-10,vmax=1e-5), cmap=plt.get_cmap('jet'))
