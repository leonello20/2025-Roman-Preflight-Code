...
class CAPyBARAsim:
    ...
    def get_system (self, influence_function, check = False):
        if self.param['path']['jacobian'] != 'None': 
            self.log.info('Read jacobian matrix: %s', self.param['path']['jacobian'])
            self.log.info('Jacobian type: %s', type(self.param['path']['jacobian']))
            ...
            raise ValueError('Jacobian path is not provided')
    ...
    def get_reference_image(self, wvl, static_aberration_func, wavefront_error = None, check = False):
        ...
        if wavefront_error is not None:
            self.log.info('Inject wavefront error')
