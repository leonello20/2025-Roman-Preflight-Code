...
def coronagraph(wfo, f_lens, occulter_type, diam, occrad):
    ...
    if zernike_coeffs is not None:
        if isinstance(zernike_coeffs, (np.ndarray, list)):
            proper.prop_zernikes(wfo, zernike_coeffs, noll=True)
            print(f"DEBUG: Applied Zernike coefficients: {zernike_coeffs}")
        else:
            print("WARNING: zernike_coeffs must be a list or numpy array if provided. No aberrations applied.")

    if offset_x != 0.0 or offset_y != 0.0:
        # proper.prop_offset_array takes offset in FPM units (lambda/D units)
        proper.prop_offset_array(wfo, offset_x, offset_y)
        print(f"DEBUG: Applied source offset: ({offset_x:.2f}, {offset_y:.2f}) lambda/D")
        if is_plotting:
            plt.figure(figsize=(6, 6))
            proper.prop_show_phase(wfo, PREPEND="Phase at Pupil with Offset")
            plt.title("Wavefront Phase at Pupil (with Source Offset)")
