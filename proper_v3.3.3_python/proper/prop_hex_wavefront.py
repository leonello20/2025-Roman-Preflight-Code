...
#   June 2024 - JEK - updated deprecated numpy types
#   Nov 2024 - JEK - fixed overlapping segment phase maps when gap is not fully resolve by averaging between segments
...
def prop_hex_wavefront(wf, nrings, hexrad, hexsep, zernike_val = 0, **kwargs):
    ...
    if include_phase_errors:
        # average in pixels where segments overlap due to poor gap resolution
        phase = np.divide( phase, totseg, where=totseg!=0 )
