...
def run_efc_segmented(hsm, dh_mask, wf_star_flat, wavefront_planet, jacobian_real, E_vec_initial, num_iterations=10, efc_loop_gain=0.3, rcond=1e-1):
    ...
    # Propagate the initial aberrated state for visualization
    ...
    wf_focal = propagate_coronagraph(wf_total_corrected, occulter_mask, lyot_stop_mask, prop, prop_inverse)

    electric_fields_history.append(wf_focal.electric_field)
    images_history.append(wf_focal.intensity)
    
    norm_hc = wf_star_corrected.total_power
    norm_ref = img_ref.max()
    initial_contrast = np.mean(wf_focal.intensity[dh_mask] / norm_ref)
    ...
    for i in range(num_iterations):
        ...
        # --- 1. MEASURE E-FIELD from CURRENT DM STATE ---
        # NOTE: hsm.actuators is already set to the state from the previous step 
        # (or initial state for i=0/step 1) via the current_actuators_1d variable.
        ...
        # Store the new actuator commands
        actuators_history.append(current_actuators_1d.copy())
        
        # --- 4. PROPAGATE NEW STATE for Visualization/Contrast Calculation ---
        hsm.actuators = current_actuators_1d # Apply new commands to DM object
        wf_star_corrected = hsm(wf_star_flat)
        wf_focal_star_corrected = propagate_coronagraph(wf_star_corrected, occulter_mask, lyot_stop_mask, prop, prop_inverse)
        
        # Total WF for visualization/contrast
        wf_total_corrected = hp.Wavefront(wf_star_corrected.electric_field + wavefront_planet.electric_field, wavelength=wavelength)
        wf_focal = propagate_coronagraph(wf_total_corrected, occulter_mask, lyot_stop_mask, prop, prop_inverse)
        ...
        # Store the TOTAL (Star + Planet) field for visualization history
        ...
        # Calculate normalization constant and contrast
        norm_ref = img_ref.max()
        print(f"Initial Star-Only Contrast: {np.mean(wf_focal_star_corrected.intensity[dh_mask] / norm_ref):.2e}")
...
wavefront_planet.total_power = sqrt_contrast**2
