...
def run_efc_simulation (CAPyBARA, aberration_class, param_rst, save=True, path=None):
    """
...
def main(mode: str, config_file: str) -> None:
    ...
    # --- START DEBUG BLOCK (Line 130) ---
    if 'path' not in param_rst:
        print("--- CONFIGURATION DEBUG START ---")
        print(f"Error: The key 'path' was not found in the loaded parameters.")
        print("Available top-level sections (keys in param_rst):")
        
        # Print all the keys the parser actually loaded. This is the crucial diagnostic step.
        for key in param_rst.keys():
            print(f"- {key}")
            
        print("\nFixes to try (in your .ini file):")
        print("1. Ensure there is NO leading whitespace or blank lines before '[path]'.")
        print("2. Try capitalizing the section: rename '[path]' to '[Path]'.")
        print("3. Check the file encoding (must be UTF-8).")
        print("--- CONFIGURATION DEBUG END ---")
        # Reraise the exception so the user can see the original trace
        raise KeyError("path")
    # --- END DEBUG BLOCK ---

    path_key = None
    if 'path' in param_rst:
        path_key = 'path'
    elif 'Path' in param_rst:
        path_key = 'Path'
    ...
    path = param_rst[path_key]['data_path']
    ...
    # CRITICAL FIX (Line 134): Define 'sequence' by loading it from the config file,
    # as it was used later without being defined, which would cause a NameError.
    if 'sequence' in param_rst:
        sequence = param_rst['sequence']
    else:
        # Fallback to prevent crash if sequence section is entirely missing
        print("WARNING: [sequence] section not found in config. Assuming default: is_efc=True, is_observation=True.")
        sequence = {'is_efc': True, 'is_observation': True}
